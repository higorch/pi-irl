...
class StreamService(QObject):
    """Coordena status, validação e ciclo de vida do GStreamer."""
    ...
    def __init__(self, parent: QObject | None = None) -> None:
        ...
        self._gstreamer = GStreamerService(self)
        self._user_stopping = False

        self._gstreamer.log_line.connect(self.log_line.emit)
        self._gstreamer.started.connect(self._on_gstreamer_started)
        self._gstreamer.finished.connect(self._on_gstreamer_finished)
        self._gstreamer.error_occurred.connect(self._on_gstreamer_error)
    ...
    def start(self, config: StreamConfig) -> bool:
        ...
        if not self._gstreamer.gstreamer_available():
            errors.append("GStreamer (gst-launch-1.0) não encontrado.")
        ...
        self._gstreamer.start(config)
        return True

    def stop(self) -> None:
        if not self.is_active and not self._gstreamer.is_running():
            return
    ...
    def _on_gstreamer_finished(
        ...
        if exit_status == QProcess.ExitStatus.CrashExit or exit_code != 0:
            ...
            self.log_line.emit(
                f"GStreamer encerrou inesperadamente (código {exit_code})."
            )
        else:
            self._set_status(StreamStatus.OFFLINE)
            self.log_line.emit("GStreamer finalizou.")

    def _on_gstreamer_error(self, message: str) -> None:
        self._set_status(StreamStatus.ERROR)
