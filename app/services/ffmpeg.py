...
class FFmpegService(QObject):
    ...
    def _build_linux_command(self, config: StreamConfig) -> list[str]:
        ...
        -i /dev/video0 -f alsa -channels 1 -i hw:3,0
        -c:v libx264 -preset ultrafast -b:v 2500k -g 60
        -c:a aac -b:a 128k -ac 1 -f mpegts
        "srt://IP:8890?mode=caller&streamid=publish:irl"
        """
        return [
            "ffmpeg",
            "-f",
    ...
    def _build_windows_command(self, config: StreamConfig) -> list[str]:
        """Comando DirectShow equivalente para testes no Windows."""
        video_input = f"video={config.camera}"
        audio_input = f"audio={config.microphone}"
